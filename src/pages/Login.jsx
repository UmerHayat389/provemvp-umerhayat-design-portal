...
const Login = ({ setUser }) => {
  ...
  const [error, setError] = useState('');
   ...
   const handleEmailChange = (e) => {
    ...
    if (error) setError(''); // clear error dynamically
  };
  ...
  const handlePasswordChange = (e) => {
    ...
    if (foundUser) {
      setUser(foundUser); // Admin or Employee
      setError('');
    } else {
          ...
          <form className="mt-8" method="POST" action="#">
            <div className="space-y-5">
              <div>
                ...
                <div className="mt-2">
                  <input placeholder="Email" type="email" className="flex h-10 w-full rounded-md border border-gray-300 bg-transparent px-3 py-2 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-400 focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50"  value={email}
              ...
              <div>
                ...
                <div className="mt-2">
                  <input placeholder="Password" type="password" className="flex h-10 w-full rounded-md border border-gray-300 bg-transparent px-3 py-2 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-400 focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50"  value={password}
            ...
            <p className="text-red-500 text-center mt-2">{error}</p>
              ...
              <div>
                <button className="inline-flex w-full items-center justify-center rounded-md bg-black px-3.5 py-2.5 font-semibold leading-7 text-white hover:bg-black/80" type="submit" onClick={handleLogin}>
                  Get started
                </button>
              </div>
            </div>
